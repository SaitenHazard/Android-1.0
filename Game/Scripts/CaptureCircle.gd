...
#	_find_segemnt_critter_intersection()
...
func _find_segment_segment_intersection():
	...
	for x in (points_line.size() - 4):
		...
		var segment_segment_intersection = Geometry.segment_intersects_segment_2d(
			latest_segment_from, latest_segment_to, segment_from, segment_to)
			
		if not segment_segment_intersection == null:
			pointC = segment_segment_intersection
			_draw_shape()
